...
<template>
  <main v-if="!auth.team" class="w-screen flex flex-col items-center justify-center">
    ...
    <div class="bg-white shadow-md mx-2 px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-3xl w-50 max-w-sm">
      ...
      <hr v-if="auth.user.email == 'fyic@miamioh.edu'" class="my-6">

      <button v-if="auth.user.email == 'fyic@miamioh.edu'" @click="() => auth.exportToCSV()" :disabled="auth.loading" class="flex items-center justify-center focus:outline-none text-white text-sm sm:text-base rounded-2xl py-2 w-full transition duration-150 ease-in" :class="auth.loading ? 'bg-gray-500 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'">
        <span class="mr-2 uppercase">Export to CSV</span>
